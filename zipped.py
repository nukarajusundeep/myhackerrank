...
try:
    student, subj = (int(subj) for subj in input().split())
    stu = []
    for _ in range(subj):
        stu_marks = [float(x) for x in input().split()]
        if len(stu_marks) > student:
            raise Exception("Values entered is more than students")
        stu.append(stu_marks)
    for sub in zip(*stu):
        print(sum(sub) / subj)
except ValueError :
    print("Exceeded input Should Input only number of Students  and number of subjects ")
